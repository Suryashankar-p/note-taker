...
export const CreateChat = async (title: string) => {
  ...
  console.log(body);
  const response = await DoctorBotAPI.post(BACKEND_DOCTOR_CONBOT_URL + `/doctor_conbot/chat/`,body);
  return response;
};
...
export const CreateAgentChatHistory = async (query: string, chat_id: string) => {
  let body={
    human: query
  }
  const response = await DoctorBotAPI.post(
    BACKEND_DOCTOR_CONBOT_URL + `/doctor_conbot/chat/${chat_id}/agent/`, body
  );
  return response;
};
...
export const ReadActivityUsageTopUsers = async (year: string | number, month: string | number, skip: number = 0, limit: number = 6) => {
  const response = await DoctorBotAPI.get(BACKEND_DOCTOR_CONBOT_URL + `/doctor_conbot/usage/activity/top?skip=${skip}&limit=${limit}&year=${year}&month=${month}`)
  return response
}
...
export const PollDocumentStatus = async (product_document_id: string | number) => {
  const response = await DoctorBotAPI.get(BACKEND_DOCTOR_CONBOT_URL + `/doctor_conbot/product/{product_id}/document/${product_document_id}/status`);
  return response;
};

//<====================================Dr. Conbot category========================================>

export const ReadCategories = async (skip: number = 0, limit: number = 100, search_term?: string) => {
  const response = await DoctorBotAPI.get(BACKEND_DOCTOR_CONBOT_URL + `/doctor_conbot/category?skip=${skip}&limit=${limit}${search_term !== '' ? '&search_term=' + search_term : ''}`)
  return response
}

export const CreateCategory = async (body: any) => {
  const response = await DoctorBotAPI.post(BACKEND_DOCTOR_CONBOT_URL + `/doctor_conbot/category/`,body);
  return response
}

export const DeleteCategory = async (product_id: string) => {
  const response = await DoctorBotAPI.delete(BACKEND_DOCTOR_CONBOT_URL + `/doctor_conbot/category/${product_id}`)
  return response
}

//<====================================Dr. Conbot category document========================================>

export const ReadCategoryDocuments = async (category_id: number | string, skip: number = 0, limit: number = 0, search_term?: string) => {
  const response = await DoctorBotAPI.get(BACKEND_DOCTOR_CONBOT_URL + `/doctor_conbot/category/${category_id}/document?skip=${skip}&limit=${limit}${search_term !== '' ? '&search_term=' + search_term : ''}`)
  return response
}
...
export const DeleteCategoryDocument = async (category_id: string | number, category_document_id: string | number) => {
  const response = await DoctorBotAPI.delete(BACKEND_DOCTOR_CONBOT_URL + `/doctor_conbot/category/${category_id}/document/${category_document_id}`)
  return response
}

export const ReadCategoryDocumentUrl = async (category_id: string | number, category_document_id: string | number) => {
  const response = await DoctorBotAPI.get(BACKEND_DOCTOR_CONBOT_URL + `/doctor_conbot/category/${category_id}/document/${category_document_id}/link`)
  return response
}
...
export const CreateFaqDocument = async (
  ...
  try {
    const response = await axios.post(
      `${BACKEND_DOCTOR_CONBOT_URL}/doctor_conbot/faq${category_document_id ? '/' + category_document_id : ''}?description=${description}&kind=FAQ`,
...
export const DeleteFaqDocument = async (category_document_id: string | number) => {
  const response = await DoctorBotAPI.delete(BACKEND_DOCTOR_CONBOT_URL + `/doctor_conbot/faq/${category_document_id}`)
  return response
}

//<====================================Dr. Conbot sub package========================================>

export const ReadSubpackages = async (category_id:number | string, skip: number = 0, limit: number = 100, search_term?: string) => {
  const response = await DoctorBotAPI.get(BACKEND_DOCTOR_CONBOT_URL + `/doctor_conbot/category/${category_id}/sub_package?skip=${skip}&limit=${limit}${search_term !== '' ? '&search_term=' + search_term : ''}`)
  return response
}

export const CreateSubpackage = async (body: any, category_id: number | string) => {
  const response = await DoctorBotAPI.post(BACKEND_DOCTOR_CONBOT_URL + `/doctor_conbot/category/${category_id}/sub_package/`, body);
  return response
}

export const DeleteSubPackage = async (category_id: number | string, sub_package_id: string | number) => {
  const response = await DoctorBotAPI.delete(BACKEND_DOCTOR_CONBOT_URL + `/doctor_conbot/category/${category_id}/sub_package/${sub_package_id}`)
  return response
}

//<====================================Dr. Conbot sub package document========================================>

export const ReadSubPackageDocuments = async (sub_package_id: number | string, skip: number = 0, limit: number = 0, search_term?: string) => {
  const response = await DoctorBotAPI.get(BACKEND_DOCTOR_CONBOT_URL + `/doctor_conbot/category/sub_package/${sub_package_id}/document?skip=${skip}&limit=${limit}${search_term !== '' ? '&search_term=' + search_term : ''}`)
  return response
}
...
export const DeleteSubPackageDocument = async (sub_package_document_id: string | number,sub_package_id: string | number) => {
  const response = await DoctorBotAPI.delete(BACKEND_DOCTOR_CONBOT_URL + `/doctor_conbot/category/sub_package/${sub_package_id}/document/${sub_package_document_id}`)
  return response
}
