...
import {
  TransmitterCreateMasterActivity,
  TransmitterDeleteMasterActivity,
...
const MasterActivityPage: React.FC<MasterActivityPageProps> = ({ onSelectActivity }) => {
  ...
  // New state for tracking activity creation
  ...
  const onUpdate = async (title: string) => {
    ...
    try {
      ...
      const response = await TransmitterUpdateMasterActivityDetails(
        defaultActivity?.id,
        payload
      );
      if (response?.id) {
  ...
  const onDeleteSubmit = async (activity: any) => {
    setIsLoading(true);
    try {
      // Check if the master activity has child activities
      const hasChildActivities = await TransmitterCheckMasterHasChildActivities(activity.id);
      if (hasChildActivities) {
        setPageError(true);
        dispatch.toast.openToast({
          status: true,
          message: "Cant delete this master activity, child activities are present under it",
          type: "error",
        });
        setIsLoading(false);
        return;
  ...
  return (
    <div className="flex flex-1 h-screen">
      ...
      <div className="flex-1 p-6 h-full">
        {/* Top section with title, filters, and search */}
        ...
        <div ref={activityListRef} className="mt-4 mb-5 flex-1 h-[calc(100vh-230px)] pr-4 overflow-y-auto">
          ...
          {/* Show fetching loader at the bottom when loading more activities */}
          {isFetching && !isCreatingActivity && (
...
export default MasterActivityPage;