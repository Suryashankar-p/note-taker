...
interface Props {
  ...
  activityStatus: any;
  reachedBottom: () => void;
  usageType: 'master' | 'child'; // Add usageType prop
}
export default function Activity1({
  ...
  const options = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `Monthly ${usageType === 'master' ? 'Master' : 'Child'} Activities: ${totalQuestions}`,
      },
      tooltip: {
        callbacks: {
  ...
  // Function to filter activity statuses
  const filterActivityStatus = (statusData: any) => {
    if (!statusData) return [];
    
    // Convert object to array if needed
    const statusArray = Array.isArray(statusData) 
      ? statusData 
      : Object.entries(statusData).map(([stat, activity_count]) => ({ stat, activity_count }));
    
    // Filter to only include the three required statuses
    return statusArray.filter((item: any) => 
      ['IN_PROGRESS', 'REJECTED', 'DELETED'].includes(item.stat)
    );
  };
  ...
  const activityStatusMapper = (status: string) => {
    switch (status) {
      ...
      case "DELETED":
        return "Deleted";
      default:
        // Return null for other statuses so they won't be displayed
        return null;
    }
  };
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {stats?.length > 0 ? (
              stats.map((stat: any, index: number) => {
                const statusName = stat.name || (stat.stat && activityStatusMapper(stat.stat));
                // Only render rows for valid statuses
  ...
  return (
    <div className="flex relative py-6 px-4 flex-row justify-between items-start xl:space-x-20">
      ...
      <div className="mt-1 xl:mr-1 flex flex-col gap-3 max-w-sm w-full">
        {renderStatsTable(`Top ${usageType === 'master' ? 'Master' : 'Child'} Users Status`, topUsers)}
        {renderStatsTable(
          `${usageType === 'master' ? 'Master' : 'Child'} Activity Status`, 
