...
const Feedback = () => {
  ...
  useEffect(() => {
    if (activeTab) {
      getAllFeedbacks(0, 20, activeTab, "");
    }
  }, [activeTab]);
  ...
  const feedBackReviewSubmit = async (data) => {
    if (data) {
        ...
        try {
            // Ensure \n newlines are correctly formatted without double escaping
            const result = JSON.stringify(data?.updated_answer);

            // Call the update function directly with the clean result (no JSON.stringify here)
            const response = await UpdateChatFeedback(
                defaultFeedback?.id,
                data?.updated_question,
                result.slice(1,-1),  // Use the cleaned-up string directly
                data?.status,
                body
            );
        if (response?.id) {
    ...
    return items?.length > 0 ? (
      items.map((item, key) => (
        ...
        >
          ...
          {/* {(salesMemberDetails?.role === 'OWNER' || salesMemberDetails?.role === 'REVIEWER') && <button onClick={() => onClick(item)} className='self-center ml-auto mr-10 bg-[#F3F1FF] p-2 rounded-full'>
