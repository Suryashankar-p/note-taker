...
import { DeleteAllChatList, ReadChatHistories, UpdateChat } from '../../services/sales';
...
const Sidebar: React.FC<Props> = ({ history = [], onChatDeleteClick, getChatList }) => {
  ...
  const handleChatClick = async (item: any) => {
    ...
    navigate(`/ai-studio/sales?chat_id=${item?.id}`);
    if (item.id != chat_id) {
      getPageChat(item.id);
    }
  };
  ...
  return (
    <div className="w-full bg-primary_text flex flex-col">
      ...
      <Button
        onClick={() => navigate('/ai-studio/sales/settings')}
