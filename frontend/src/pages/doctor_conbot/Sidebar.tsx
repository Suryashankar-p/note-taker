...
import { DeleteAllChatList, ReadChatHistories, UpdateChat } from '../../services/doctor_conbot.ts';
...
const Sidebar: React.FC<Props> = ({ history = [], onChatDeleteClick, getChatList, onSideBarclose }) => {
  ...
  const handleChatClick = async (item: any) => {
    ...
    navigate(`/ai-studio/doctor_conbot?chat_id=${item?.id}`);
    if (item.id != chat_id) {
      getPageChat(item.id);
    }
  };
  ...
  return (
    <div className="w-full bg-primary_text flex space-y-2 md:space-y-1 flex-col p-2 md:p-1 mt-16 md:mt-2">
      ...
      <Button
        onClick={() => navigate("/ai-studio/doctor_conbot/settings")}
