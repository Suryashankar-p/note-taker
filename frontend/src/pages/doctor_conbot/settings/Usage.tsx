...
import { ReadActivityUsage, ReadActivityUsageTopUsers, ReadCostUsage, ReadUsageLimit, UpdateUsageLimit } from '../../../services/doctor_conbot';
          ...
          <TabList className="flex space-x-2">
            {tabs.map(tab => (
              <Tab
                key={tab.key}
                className={({ selected }) =>
                  `px-4 py-2 rounded-md transition-colors duration-300 border-danger ${selected ? 'bg-danger text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`
                }
