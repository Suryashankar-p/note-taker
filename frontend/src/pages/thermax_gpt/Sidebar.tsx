...
const Sidebar: React.FC<Props> = ({
  ...
  const [editChat, setEditChat] = useState<EditType>({
    status: false,
    index: null,
  });
  ...
  const handleChatClick = async (item: any) => {
    ...
    navigate(`/ai-studio/thermax_gpt?chat_id=${item?.id}`);
    if (item.id != chat_id) {
      dispatch.chatContent.clearChat();
      getPageChat(item.id);
    }
  ...
  return (
    <div className="w-full bg-primary_text flex flex-col pt-16 h-full">
      <Button
        onClick={handleNewChatClick}
        size="medium"
        custom_type="secondary"
        className="mx-[1vw] mt-6 flex items-center"
      >
      ...
      <div className="mb-5 max-h-[53vh] overflow-y-auto overflow-x-hidden">
        {Object.keys(groupedHistory).map((category, categoryIndex) => (
          <div key={categoryIndex} className="mb-6">
            <Text
              className="text-background mt-4 mb-2 text-start pl-8 w-full border-b-2 border-[#EFEFEF]"
              type="small"
            >
              {category}
            </Text>
                  ...
                  <Button
                    ...
                    className={`border-none hover:bg-[#373737] ${
                      item.id === selectedItem?.id
                        ? "bg-[#373737]"
                        : "bg-inherit"
                    } h-11 my-2 flex items-center gap-1 justify-between w-full group`}
                  >
                    ...
                    <div className="flex opacity-0 group-hover:opacity-100 ml-10">
                      <DropDownMenu
      ...
      <div className="fixed bottom-[23vh] left-0 right-[80vw] border-b-2 border-[#EFEFEF]" />
      <div
        onClick={() => dispatch.modal.openConfirmation()}
        className="flex self-start flex-row ml-[3.5vw] bottom-[17vh] space-x-2 hover:cursor-pointer fixed"
      >
        <img src={DeleteIcon} alt="Clear All Chats" className="w-4 h-4" />
        <Text className="text-white" type="small">
          Clear Conversation
        </Text>
      </div>
      <Button
        onClick={() => navigate("/ai-studio/thermax_gpt/settings")}
        className="mx-[0.5vw] flex self items-start left-[0.5vw] w-[18vw] bottom-[5vh] space-x-2 fixed"
        custom_type="secondary"
        rounded
        size="medium"
      >
