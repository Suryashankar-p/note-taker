...
const FileEditModal: React.FC<Props> = ({ defaultValues, options, onSubmit }) => {
    ...
    useEffect(() => {
        if (defaultValues) {
            ...
            setSelectedFileType({ name: fileTypeSelctor(defaultValues?.kind) })
                                            ...
                                            <div className="flex flex-row mt-2 w-full gap-5 justify-between">
                                                <div className="flex flex-col w-2/3 gap-1">
                                                    ...
                                                    <div className="relative w-full">
                                                        <input
                                                            type="text"
                                                            {...register("fileName", { required: "Field is required" })}
                                                            className={`border ${errors?.fileName && 'border-danger'} rounded-md w-full border-grey h-12 flex-grow focus:outline-none p-4`}
                                                    ...
                                                    <Text type='small' className='text-danger'>{errors?.fileName?.message}</Text>
                                                </div>
                                                <div className="w-1/3 -h-1">
                                                    <DropDownButton
                                                        ...
                                                        onChange={(value: any) => {
                                                            ...
                                                            // Clear error when dropdown value changes
                                                            clearErrors('fileType');
                                                            setValue('fileType', value.name); // Set value for react-hook-form
                                                        }}
                                                        error={errors.fileType} // Pass the error message for validation
                                                    />
                                                </div>
                                            </div>
                                            <div className="w-full flex-col gap-5 w-full">
                                                ...
                                                <SelectTagInput
                                                    ...
                                                    onChange={(tags: Tag[]) => {
                                                        ...
                                                        setValue('models', tags); // Set value for react-hook-form
                                                    }}
                                                    options={options}
                                                />
                                                <Text type='small' className='text-danger'>{errors?.models?.message}</Text>
                                            </div>
                                            <div className="w-full flex-col gap-5 mt-3 h-32 w-full">
                                                <label><Text className='text-primary_text'>Description*</Text></label>
                                                <Textarea
                                                    className={`w-full h-full border mt-1 ${errors?.description && 'border-danger'} rounded-md focus:outline-none p-4`}
                                                    onClick={(e) => e.stopPropagation()}
