import { Dialog, Transition, DialogTitle, DialogPanel, TransitionChild, Textarea } from '@headlessui/react';
...
const FeedbackReview: React.FC<Props> = ({ productsList = [], defaultValue, formSubmit }) => {
    const feedbackReviewState = useSelector((state: RootState) => state.modal.feedbackReview);
    ...
    const checkDisabled = () => {
        if (defaultValue?.status === 'APPROVED' || defaultValue?.status === 'REJECTED') return true
                    ...
                    <div className="flex min-h-full items-center justify-center p-4 text-center">
                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    <Text>{type === 'add' ? "Add New Feedback" : "Edit Feedback"}</Text>
                                ...
                                <div className="mt-2 relative h-screen">
                                    <div className="flex border-b border-gray-200">
                                        <button
                                            className={`py-2 px-4 text-sm font-medium ${activeTab === 'feedback' ? 'border-b-2 border-danger text-danger' : 'text-gray-500'
                                                }`}
                                        ...
                                        <button
                                            className={`py-2 px-4 text-sm font-medium ${activeTab === 'review' ? 'border-b-2 border-danger text-danger' : 'text-gray-500'
                                                }`}
                                    ...
                                    <div className="mt-2">
                                        {activeTab === 'feedback' && (
                                            <div className='flex flex-col mt-2 mx-2 p-2 gap-2'>
                                                <div className='border rounded-lg overflow-y-auto w-full bg-[#F3F1FF] h-[16vh] p-1 border-black-800'>
                                                    <Text type='body' className='text-primary_text'>Question:</Text>
                                                    <Text type='small' className='text-primary_text mx-2'>
                                                        {defaultValue?.question}
                                                    </Text>
                                                </div>
                                                <div className='border rounded-lg overflow-y-auto w-full h-[16vh] bg-[#F3F1FF] p-1 border-black-800'>
                                                    <Text type='body' className='text-primary_text'>Answer:</Text>
                                                    <Text type='small' className='text-primary_text mx-2'>
                                                        {defaultValue?.answer}
                                                    </Text>
                                                ...
                                                <div className='flex flex-col justify-start gap-1'>
                                                    <div className='flex flex-row items-center gap-2'>
                                                        <label>
                                                            <Text className='text-primary_text' type='bold-body'>Status:</Text>
                                                        </label>
                                                        <div className='flex flex-row gap-2'>
                                                            <div className={`${bgFinder(defaultValue?.like)} rounded-full p-2 border border-gray-200 text-primary_text`}><Text type='small'>{defaultValue?.like === true ? 'Liked' : defaultValue?.like === false ? 'Disliked' : 'Not Specified'}</Text></div>
                                                        </div>
                                                    </div>
                                                    {defaultValue?.like === false && <div className='flex flex-col items-start'>
                                                        <label>
                                                            <Text className='text-primary_text' type='bold-body'>Dislike Reason:</Text>
                                                        </label>
                                                        <div className='flex flex-row gap-2'>
                                                            {dislikeReason?.map((item) => (
                                                                <div key={item.id} className='rounded-lg p-2 border bg-[#F3F1FF] border-gray-200 h-[15vh] overflow-y-auto w-[50vw] text-primary_text'><Text className='text-primary_text' type='small'>{item.name}</Text></div>
                                                            ))}
                                                        </div>
                                                    </div>}
                                                </div>
                                        ...
                                        {activeTab === 'review' && (
                                            <form onSubmit={handleSubmit(onSubmit)}>
                                                ...
                                                <div className='flex flex-col m-2 p-2 gap-4'>
                                                    <div className='flex flex-row gap-4 '>
                                                        <div className='w-1/2 flex flex-col gap-2'>
                                                            <label><Text className='text-primary_text'>Edit question</Text></label>
                                                            <Textarea
                                                                ...
                                                                {...register('updated_question', { required: "Field is required" })}
                                                            />
                                                            {errors.updated_question && <span className="text-red-500">{errors.updated_question.message}</span>}
                                                        </div>
                                                        <div className='w-1/2 flex flex-col gap-2'>
                                                            <label><Text className='text-primary_text'>Edit answer</Text></label>
                                                            <Textarea
                                                                className={`w-full text-primary_text h-[20vh] border rounded-md focus:outline-none p-4 'border-gray-300'`}
                                                                defaultValue={defaultValue?.updated_answer}
                                                                disabled={checkDisabled()}
                                                                {...register('updated_answer', { required: "Field is required" })}
                                                            />
                                                            {errors.updated_answer && <span className="text-red-500">{errors.updated_answer.message}</span>}
                                                        </div>
                                                    </div>
                                                    <div className='flex flex-row gap-4'>
                                                ...
                                                <div className="fixed right-10 bottom-8 flex space-x-4">
                                                    ...
                                                    {salesMemberDetails?.role === 'OWNER' ? <div className='flex flex-row gap-4'>
                                                        {defaultValue?.status !== 'APPROVED' && <button
                                                            type='submit'
                                                        ...
                                                        {defaultValue?.status !== 'REJECTED' && <button
                                                            type='submit'
                                                            className="py-2 px-4 bg-[#0061F3] text-white rounded-md"
                                                        ...
                                                        salesMemberDetails?.role === 'REVIEWER' && defaultValue?.status === 'NOT_REVIEWED' && <button
                                                            type='submit'
                                                            className="py-2 px-4 bg-[#0061F3] text-white rounded-md"
