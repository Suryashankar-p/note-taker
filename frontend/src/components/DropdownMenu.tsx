...
const DropDownMenu: React.FC<DropDownMenuProps> = ({ content, menuItems, position = "bottom end", onChange, profile }) => {
    ...
    return (
        <div className="relative">
            <Menu>
                <MenuButton onClick={(e) => e.stopPropagation()} className="inline-flex items-center gap-2 rounded-md py-1.5 px-3 focus:outline-none">
                    {content}
                </MenuButton>
                <Transition
                ...
                >
                    <MenuItems
                        anchor={position}
                        className={`absolute w-${profile ? '68' : 'fit'} z-100 rounded-xl shadow-custom border border-grey bg-background p-1 text-[14px] text-primary_text data-[focus]:text-black [--anchor-gap:var(--spacing-1)] focus:outline-none`}
                    >
                        {profile ? ( // Render profile menu items if profile parameter is true
                            <>
                                <MenuItem>
                                    <div className="px-3 py-2 flex items-center">
                                        <div className="w-8 h-8 bg-gray-200 rounded-full flex items-center justify-center mr-3">
                                            <span className="text-gray-600">{userData && getInitials(userData.name)}</span>
                                        </div>
                                        <div className="ml-3 overflow-hidden">
                                            <Text type="small" className="font-bold">{userData?.name}</Text>
                                            <Text
                                                type="small"
                                ...
                                <MenuItem>
                                    ...
                                    >
                                        <img className='w-6 h-6' src={LogOut} loading='lazy' alt='logout' />
                                        <Text type="small" className="text-danger">Logout</Text>
                                    </button>
                                </MenuItem>
                            </>
                        ) : (
